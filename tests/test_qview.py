...
from mbq.api_tools.qviews import QView, qview
...
class ViewFunctionTests(SimpleTestCase):
    ...
    def test_permissions_required(self):
        with self.assertRaises(TypeError):
            qview(permissions=[])(view_func)

        response = qview(permissions=[permissions.NoAuthorization])(view_func)(
            self.request
        )
    ...
    def test_perform_authorization_drf_class(self):
        self.assertTrue(
            isinstance(
                qview(permissions=[TruePermissionStub])(view_func)(self.request),
                DetailResponse,
            )
        )

        response = qview(permissions=[FalsePermissionStub])(view_func)(self.request)

        self.assertTrue(isinstance(response, UnauthorizedResponse))
        self.assertTrue(response.data["detail"], "Authorization Failed")

        response = qview(permissions=[FalsePermissionWithMessageStub])(view_func)(
            self.request
        )
        ...
        self.assertTrue(
            isinstance(
                qview(permissions=[FalsePermissionStub, TruePermissionStub])(view_func)(
                    self.request
                ),
                UnauthorizedResponse,
            )
    ...
    def test_perform_authorization_function(self):
        wrapped_view = qview(permissions=[lambda x: True])(view_func)
        response = wrapped_view(self.request)
        self.assertTrue(isinstance(response, DetailResponse))

        wrapped_view = qview(permissions=[lambda x: False])(view_func)
        response = wrapped_view(self.request)
        self.assertTrue(isinstance(response, UnauthorizedResponse))
    ...
    def test_method_not_allowed(self):
        self.assertTrue(
            isinstance(
                qview("POST", permissions=[TruePermissionStub], payload={})(view_func)(
                    self.request
                ),
                MethodNotAllowedResponse,
            )
        ...
        self.assertTrue(
            isinstance(
                qview("GET", permissions=[TruePermissionStub])(view_func)(self.request),
                DetailResponse,
            )
        )
    ...
    def test_params(self):
        ...
        decorated_view = qview(permissions=[TruePermissionStub], params=param_schema)(
            view_func
        )
...
class TestView(QView):
    @qview.method("POST", permissions=[TruePermissionStub], payload={})
